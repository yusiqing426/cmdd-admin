...
printerModule.controller('printerListController', 
	...
	$scope.savePrinter = function(){
		console.log("$scope.kitchenPrinter");
		console.log($scope.kitchenPrinter);
		...
		//	$scope.kitchenPrinter.sync_status = syncStatus+1;

		    PrinterService.create($scope.kitchenPrinter,function(response){
			    if(response.code!=200){
			    	 alert("创建失败");
			    	 return;
			    }else{
					...
					//	$scope.orderPrinter.sync_status = syncStatus+1;
					...
					///if($scope.orderPrinter.sync_status!=syncStatus+1){
    				//		$scope.orderPrinter.sync_status = syncStatus+2;
					//	}
						PrinterService.update($scope.orderPrinter,function(response){
							if(response.code != 200){
								alert("更新失败");
								return;
							}else{
		...
		//	if($scope.kitchenPrinter.sync_status!=syncStatus+1){
    	//		$scope.kitchenPrinter.sync_status = syncStatus+2;
		//	}
			PrinterService.update({id:$scope.kitchenPrinter.id},$scope.kitchenPrinter,function(response){
				if(response.code != 200){
					alert("更新失败");
					return;
				}else{
					...
					//	$scope.orderPrinter.sync_status =syncStatus+1;
						...
						PrinterService.create($scope.orderPrinter,function(response){
							if(response.code!=200){
								 alert("创建失败");
								 return;
							}else{
    				...
    				//		$scope.orderPrinter.sync_status = syncStatus+2;
					//	}
						PrinterService.update({id:$scope.orderPrinter.id},$scope.orderPrinter,function(response){
							if(response.code != 200){
								alert("更新失败");
								return;
							}else{
...
printerModule.controller('printerCreateController', [ '$scope', '$location', 'PrinterService','DataUtilService','CategoryService',
	function ($scope, $location, PrinterService,DataUtilService,CategoryService) {
		...
		$scope.create = function () {
			...
			$scope.printer.sync_status = printer+1;
		    PrinterService.create($scope.printer,
			   function(response){
			         if(!response==200){
			        	 alert("创建失败");
			        	 return;
...
printerModule.controller('printerController',['$scope','$location','PrinterService','$routeParams','DataUtilService','CategoryService',
	function($scope,$location,PrinterService,$routeParams,DataUtilService,CategoryService){
		...
		PrinterService.view({id:printerId},function(response){
    ...
    //			$scope.orderPrinter.sync_status = syncStatus+2;
		//	}
			PrinterService.update($scope.printer,
					function(response){
						if(response.code!=200){
							alert("更新失败");
							return;
